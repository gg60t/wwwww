...
from sbb_b import BOTLOG, BOTLOG_CHATID, Config, sbb_b

# from . import BOTLOG, BOTLOG_CHATID
from sbb_b.assistant.botmanagers import ban_user_from_bot
...
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        buttons = [
            [
                Button.url("• السورس •", "https://t.me/qlv88"),
...
@sbb_b.bot_cmd(pattern="^معلومات$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
...
async def send_flood_alert(user_) -> None:
    ...
    if found:
        ...
        try:
            ...
            if BOTLOG:
                await sbb_b.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
